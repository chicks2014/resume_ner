...
class text_extracter:
    ...
    def __init__(self, resume_source, resume_extracted, resume_cleaned, resume_not_extracted):
        self.resume_source = resume_source
        self.resume_extracted = resume_extracted
        self.resume_cleaned = resume_cleaned
        self.resume_not_extracted = resume_not_extracted
    ...
    def text_extraction_image_pdf(self, resume_file_path):
    ...
    def text_extraction(self):
        ...
        try:
            ...
            for process_file in os.listdir(self.resume_source):
                file, extension = os.path.splitext(process_file)
                pInvalidPDFFile = False
                pInvalidFile = False
                try:
                    resume_file_path = os.path.join(self.resume_source, process_file)
                    ...
                    if (pInvalidPDFFile):
                        try:
                            if (resume_file_path.endswith(".pdf")):
                                pTextString = self.text_extraction_image_pdf(resume_file_path)
                                pTextStringLength = len(pTextString)
                    ...
                    if (pInvalidFile):
                        try:
                            presume_source_file_path = os.path.join(self.resume_source, process_file)
                            shutil.copy2(presume_source_file_path, self.resume_not_extracted)
                        ...
                        try:
                            ...
                            # file.write(pTextString)
                            # file.close()
                            with open(generatedFile, 'w', encoding="utf-8") as f:
                                f.write(pTextString)
    ...
    def text_cleaning(self):
        try:
            ...
            print("Source Folder : \n")
            for process_file in os.listdir(self.resume_extracted):
                file, extension = os.path.splitext(process_file)
                try:
                    resume_extracted_file_path = os.path.join(self.resume_extracted, process_file)
